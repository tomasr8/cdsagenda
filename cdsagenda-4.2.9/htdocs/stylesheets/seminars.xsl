...
<tr>
	<td valign="top" align="left">
		<xsl:call-template name="agendatitle">
			<xsl:with-param name="agenda" select="."/>
		</xsl:call-template>
		<font size="-2" color="#000060">
		<br/>
		[ http://doc.cern.ch/age?<xsl:value-of select="@id"/> ]<br/>
		[ last update: 
	...
	<td align="right" valign="top">
		<xsl:call-template name="daylist">
			<xsl:with-param name="agenda" select="."/>
		</xsl:call-template>
	</td>
</tr>
...
<tr>
	...
	<td align="right" valign="bottom">
		<xsl:call-template name="shortdaylist">
		</xsl:call-template>
		<a href="#top"><font size="-2">top <img src="images/upArrow.gif" border="0"/></font></a>
	</td>
</tr>
<tr>
	<td colspan="4">
...
&#60;tr bgcolor="#90C0F0" class="header"&#62;
	...
	<br/>
	<a href="makevCal.php?id={../../@id}{../@id}{@id}">
	   <img src="images/note.gif" width="20" height="20" hspace="0" vspace="0" alt="Add to my personal scheduler" border="0"/>
	</a>
...
<tr>
	...
	<xsl:when test="count(preceding-sibling::session) mod 2 = 1">
		<xsl:text disable-output-escaping="yes">
		&#60;td valign="top" bgcolor="#E4E4E4" colspan="4"&#62;
		</xsl:text>
	</xsl:when>
	<xsl:otherwise>
		<xsl:text disable-output-escaping="yes">
		&#60;td valign="top" bgcolor="#F6F6F6" colspan="4"&#62;
		</xsl:text>
	</xsl:otherwise>
	</xsl:choose>
	<font size="-2">
...
<xsl:template name="shortdaylist">
	<xsl:for-each select="../session">
<xsl:variable name="day" select="@sday"/>
<xsl:if test="count(preceding-sibling::session[position()=1 and @sday=$day]) = 0">
	<font size="-2">
